...
function truncate(value, width) {
    ...
    if (width <= 1)
        return normalized.slice(0, Math.max(width, 0));
...
function splitCommandLine(command) {
    ...
    if (escaped)
        current += "\\";
    if (quote)
        throw new Error("Commande agent invalide: guillemet non ferme.");
    if (current)
        result.push(current);
    return result;
}
...
class KanbanStore {
    ...
    async load() {
        try {
            ...
            throw new Error(`Impossible de lire ${this.filePath}: ${error.message}`);
        }
    }
    async save(board) {
...
class Runtime {
    constructor(store) {
        this.store = store;
        this.tmux = process.env.PK_KANBAN_TMUX_COMMAND?.trim() || "tmux";
        this.agentTemplate = process.env.PK_KANBAN_AGENT_COMMAND?.trim()
            || "opencode --prompt {prompt}";
    }
    sessionName(card) {
        if (card.sessionName)
            return card.sessionName;
    ...
    hasSession(card) {
        const result = (0, child_process_1.spawnSync)(this.tmux, ["has-session", "-t", this.sessionName(card)], {
            stdio: "ignore"
        });
        return result.status === 0;
    }
    activeSessions() {
        const result = (0, child_process_1.spawnSync)(this.tmux, ["list-sessions", "-F", "#{session_name}"], {
            encoding: "utf8"
        });
        if (result.status !== 0)
            return new Set();
    ...
    async ensureSession(card) {
        const sessionName = this.sessionName(card);
        card.sessionName = sessionName;
        if (this.hasSession(card))
            return { created: false, message: "Session existante conservee." };
        ...
        if (command.length === 0)
            throw new Error("La commande agent est vide.");
        const shellCommand = command.map(shellQuoteToken).join(" ");
        const result = (0, child_process_1.spawnSync)(this.tmux, [
    ...
    attach(card) {
        if (!this.hasSession(card))
            throw new Error("Aucune session active pour cette carte.");
        (0, child_process_1.spawnSync)(this.tmux, ["attach-session", "-t", this.sessionName(card)], { stdio: "inherit" });
    }
    feedback(card, message) {
        ...
        if (result.status !== 0)
            throw new Error(result.stderr?.trim() || "Envoi impossible.");
    }
    kill(card) {
        if (!this.hasSession(card))
            return false;
        const result = (0, child_process_1.spawnSync)(this.tmux, ["kill-session", "-t", this.sessionName(card)], {
            encoding: "utf8"
        });
        if (result.status !== 0)
            throw new Error(result.stderr?.trim() || "Arret impossible.");
        return true;
    }
}
class KanbanTui {
    constructor(store) {
        ...
        this.render = () => {
            ...
            lines.push("", `${BOLD}N${RESET} nouvelle  ${BOLD}E${RESET} modifier  ${BOLD}[ ]${RESET} deplacer  ${BOLD}Enter${RESET} ouvrir  ${BOLD}R${RESET} revue  ${BOLD}X${RESET} arreter  ${BOLD}Q${RESET} quitter`, this.message ? `${COLORS.in_review}${truncate(this.message, width)}${RESET}` : "");
            process.stdout.write(lines.join("\n"));
        };
        this.onKeypress = async (_text, key) => {
            if (this.busy || this.handlingKey)
                return;
    ...
    cards(status = STATUSES[this.column]) {
    ...
    async deleteCard() {
        const card = this.selected();
        if (!card || !(await this.confirm(`Supprimer definitivement « ${card.title} » ?`)))
            return;
        if (this.runtime.hasSession(card)) {
            if (!(await this.confirm("Une session est active. La tuer aussi ?")))
                return;
    ...
    async moveCard(direction) {
        ...
        if (target === "done" && !(await this.confirm("Passer en Done et fermer sa session ?")))
            return;
    ...
    async reviewMenu() {
        ...
        if (action.toLowerCase() === "d") {
            ...
            if (!commitMessage || !(await this.confirm("Indexer tous les changements et creer le commit ?")))
                return;
            ...
            if (!(await this.confirm("Pousser la branche courante ?")))
                return;
    ...
    async stopRuntime() {
        ...
        if (!(await this.confirm(`Arreter la session ${card.sessionName} ?`)))
            return;
        this.runtime.kill(card);
    ...
    async record(cardId, type, detail) {
        await this.mutate((board) => {
    ...
    runChecked(command, args) {
        const result = (0, child_process_1.spawnSync)(command, args, { cwd: this.store.workspace, encoding: "utf8" });
        if (result.status !== 0)
            throw new Error(result.stderr?.trim() || `${command} a echoue.`);
    }
    async runVisible(command, args) {
        this.busy = true;
        if (process.stdin.isTTY)
            process.stdin.setRawMode(false);
        process.stdout.write("\u001b[2J\u001b[H");
        (0, child_process_1.spawnSync)(command, args, { cwd: this.store.workspace, stdio: "inherit" });
        await this.waitForEnter();
        if (process.stdin.isTTY)
            process.stdin.setRawMode(true);
        this.busy = false;
    }
    async showHistory(card) {
        this.busy = true;
        if (process.stdin.isTTY)
            process.stdin.setRawMode(false);
        process.stdout.write("\u001b[2J\u001b[H");
...
async function main() {
    ...
    if (!workspace)
        throw new Error("Usage: kanbanTui.js --workspace <dossier>");
    const stat = await fs.stat(workspace);
    if (!stat.isDirectory())
        throw new Error(`Workspace invalide: ${workspace}`);
    await new KanbanTui(new KanbanStore(path.resolve(workspace))).run();
}
if (require.main === module) {
    void main().catch((error) => {
        process.stderr.write(`PK Kanban: ${error.message}\n`);
        process.exitCode = 1;
    });
}
//# sourceMappingURL=kanbanTui.js.map