...
let currentSidebarIconSize: number | undefined;
...
function getAgentHistoryEntries(context: vscode.ExtensionContext): AgentHistoryEntry[] {
  ...
  return raw
    .map((v) => v as Partial<AgentHistoryEntry>)
    .filter((v) => typeof v?.command === "string" && typeof v?.createdAt === "number")
    .map((v) => ({
      id: typeof v.id === "string" && v.id ? v.id : (v.command as string).toLowerCase(),
      provider: (v.provider as AgentProviderId) ?? "unknown",
      command: (v.command as string).trim(),
...
async function upsertAgentHistoryEntry(
  context: vscode.ExtensionContext,
  entry: Omit<AgentHistoryEntry, "id">
): Promise<AgentHistoryEntry> {
  const existing = getAgentHistoryEntries(context);
  const command = entry.command.trim();
  const id = command.toLowerCase();
  const next: AgentHistoryEntry = { ...entry, command, id };
...
function inferProviderFromCommand(command: string): AgentProviderId {
  const c = command.trim().toLowerCase();
  if (c.startsWith("codex ")) return "codex";
  if (c.startsWith("claude ")) return "claude";
  if (c.startsWith("gemini ")) return "gemini";
  if (c.startsWith("opencode ")) return "opencode";
  return "unknown";
}
...
class AgentHistoryItem extends vscode.TreeItem {
  constructor(
    public readonly entry: AgentHistoryEntry
  ) {
...
class AgentHistoryProvider implements vscode.TreeDataProvider<AgentHistoryItem> {
  ...
  getTreeItem(element: AgentHistoryItem): vscode.TreeItem {
    return element;
  }

  getChildren(): AgentHistoryItem[] {
...
type CodexResumeEntry = {
  id: string;
  command: string; // e.g. "codex resume xxx"
  label?: string;
  createdAt: number; // epoch ms
};

function extractCodexResumeCommand(text: string): string | null {
...
function getCodexResumeEntries(context: vscode.ExtensionContext): CodexResumeEntry[] {
  const raw = context.globalState.get<unknown>(CODEX_RESUME_STORAGE_KEY);
  if (!Array.isArray(raw)) return [];
  return raw
    .map((v) => v as Partial<CodexResumeEntry>)
    .filter((v) => typeof v?.command === "string" && typeof v?.createdAt === "number")
    .map((v) => ({
      id: typeof v.id === "string" && v.id ? v.id : (v.command as string).toLowerCase(),
      command: (v.command as string).trim(),
...
async function addCodexResumeEntry(
  context: vscode.ExtensionContext,
  entry: Omit<CodexResumeEntry, "id">
): Promise<CodexResumeEntry> {
  const existing = getCodexResumeEntries(context);
  const command = entry.command.trim();
  const id = command.toLowerCase();
  const next: CodexResumeEntry = { ...entry, command, id };
...
function scoreSession(queryTokens: Set<string>, docTokens: string[]): number {
...
async function readCodexHistory(): Promise<CodexHistoryEntry[]> {
  ...
  try {
    ...
    for (const line of content.split("\n")) {
      ...
      if (!trimmed) continue;
      try {
        const obj = JSON.parse(trimmed) as Partial<CodexHistoryEntry>;
        if (
...
function getCodexHistoryLastTs(context: vscode.ExtensionContext): number {
  const raw = context.globalState.get<unknown>(CODEX_HISTORY_LAST_TS_KEY);
  return typeof raw === "number" && isFinite(raw) ? raw : 0;
}

async function setCodexHistoryLastTs(context: vscode.ExtensionContext, ts: number): Promise<void> {
  await context.globalState.update(CODEX_HISTORY_LAST_TS_KEY, ts);
}

async function importCodexSessionsFromHistory(
  context: vscode.ExtensionContext,
...
async function readCodexSessionIndex(): Promise<CodexSessionIndexEntry[]> {
  ...
  try {
    ...
    for (const line of content.split("\n")) {
      ...
      if (!trimmed) continue;
      try {
        const obj = JSON.parse(trimmed) as Partial<CodexSessionIndexEntry>;
...
async function suggestCodexSessions(
  query: string
): Promise<Array<{ id: string; threadName?: string; updatedAt?: string; score: number }>> {
  const [sessions, history] = await Promise.all([readCodexSessionIndex(), readCodexHistory()]);
  if (!sessions.length || !history.length) return [];

  const qTokens = new Set(tokenizeForMatch(query));
  if (!qTokens.size) return [];

  const sessionToName = new Map<string, CodexSessionIndexEntry>();
  for (const s of sessions) sessionToName.set(s.id, s);

  // Build a light per-session token bag from recent history lines
  const bySession = new Map<string, { tokens: string[]; lastTs: number }>();
  for (const h of history) {
    ...
    if (!existing) {
      ...
      if (h.ts > existing.lastTs) existing.lastTs = h.ts;
  ...
  const scored: Array<{ id: string; threadName?: string; updatedAt?: string; score: number }> = [];
...
const LAUNCHPAD_THEMES: LaunchpadTheme[] = [
  "sleek",
  "classic",
  "catppuccin-mocha",
  "catppuccin-macchiato",
...
function isLaunchpadTheme(value: unknown): value is LaunchpadTheme {
  return typeof value === "string" && (LAUNCHPAD_THEMES as string[]).includes(value);
}

function getLaunchpadTheme(): LaunchpadTheme {
...
const LAUNCHPAD_THEME_LABELS: Record<LaunchpadTheme, string> = {
  "sleek": "Sleek (défaut)",
  "classic": "Classique",
...
async function removeProjectFromLaunchpad() {
  ...
  const updatedProjects = projects.filter(
    (p) => path.normalize(p.path) !== path.normalize(selected.project.path)
  );
...
async function getProjectIcon(project: LaunchpadProject): Promise<string> {
...
async function safeWriteTextFile(filePath: string, content: string) {
...
async function buildLaunchpadHtml(webview: vscode.Webview, projects: LaunchpadProject[]): Promise<string> {
  ...
  if (currentSidebarIconSize === undefined) {
    currentSidebarIconSize = vscode.workspace.getConfiguration("pkvsconf").get<number>("launchpad.sidebarIconSize") ?? 60;
  }
...
class ProjectNotesViewProvider implements vscode.WebviewViewProvider {
  private currentView: vscode.WebviewView | undefined;
...
function getTitlebarColorGlobalMap(context: vscode.ExtensionContext): Record<string, string> {
  const raw = context.globalState.get<unknown>(TITLEBAR_COLOR_GLOBAL_MAP_KEY);
...
async function setTitlebarColorGlobalMap(context: vscode.ExtensionContext, map: Record<string, string>): Promise<void> {
...
async function pickNextTitlebarColor(context: vscode.ExtensionContext): Promise<string> {
  ...
  const candidates = TITLEBAR_COLOR_PALETTE.filter(
    (c) => !recent.has(c.toLowerCase()) && !usedByOthers.has(c.toLowerCase())
  );
...
interface GitRepositoryState {
  remotes: GitRemote[];
  indexChanges: GitChange[];  // Fichiers staged
}
...
// Interface pour les résultats de scan des fichiers staged
interface StagedFileSecretResult {
  filePath: string;
  relativePath: string;
  secrets: SecretMatch[];
}
...
async function scanStagedFilesForSecrets(): Promise<{
  hasSecrets: boolean;
  files: StagedFileSecretResult[];
  ...
  const filesWithSecrets: StagedFileSecretResult[] = [];
...
async function addFilesToGitignore(
  workspaceRoot: string,
  filePaths: string[]
): Promise<void> {
...
async function unstageFiles(filePaths: string[]): Promise<void> {
  for (const filePath of filePaths) {
    try {
...
async function getGitRepoRootsFromGitApi(): Promise<{ label: string; rootUri: vscode.Uri }[]> {
  const gitExtension =
    vscode.extensions.getExtension<GitExtension>("vscode.git");
  if (!gitExtension) {
    return [];
...
let sizeDecorChannel: vscode.OutputChannel | undefined;
function sizeDecorLog(msg: string): void {
  if (!sizeDecorChannel) {
...
interface SizeTreeEntry {
  uri: vscode.Uri;
  isDirectory: boolean;
  isWorkspaceRoot?: boolean;
}

class SizeExplorerProvider implements vscode.TreeDataProvider<SizeTreeEntry> {
  private readonly _onDidChangeTreeData = new vscode.EventEmitter<
    SizeTreeEntry | undefined
  >();
  readonly onDidChangeTreeData = this._onDidChangeTreeData.event;

  private readonly dirSizeCache = new Map<string, number>();
  private readonly dirSizePromises = new Map<string, Promise<number>>();
  ...
  refreshPath(_fsPath: string): void {
    this.refresh();
  }

  async getChildren(element?: SizeTreeEntry): Promise<SizeTreeEntry[]> {
  ...
  async getTreeItem(element: SizeTreeEntry): Promise<vscode.TreeItem> {
  ...
  private async readDirectory(uri: vscode.Uri): Promise<SizeTreeEntry[]> {
    let entries: Dirent[];
    try {
  ...
  async getSize(uri: vscode.Uri): Promise<number> {
  ...
  getCachedDirectorySize(fsPath: string): number | undefined {
    return this.dirSizeCache.get(fsPath);
  }
}

class NativeSizeDecorationProvider implements vscode.FileDecorationProvider {
...
function makeSizeBadge(bytes: number): vscode.FileDecoration {
  return {
...
function formatBytesBadge(bytes: number): string {
  if (bytes < 10) return `${bytes}`;
  ...
  if (rounded >= 1 && rounded <= 9) return `${rounded}${units[i]}`;
  return units[i];
}
...
class WorkspaceSizesWebviewProvider implements vscode.WebviewViewProvider {
  private _view?: vscode.WebviewView;
  private _filter = "";
  private _dirCache = new Map<string, SizeEntry[]>();
  private _filterTimer: NodeJS.Timeout | undefined;
  private _materialMapping: { fileExtensions?: Record<string, string>; fileNames?: Record<string, string>; folderNames?: Record<string, string> } | null = null;
  private _materialExtPath: string | undefined;
  private _iconCache = new Map<string, string>();
  private _materialLoaded = false;

  private async loadMaterialIcons(): Promise<void> {
    if (this._materialLoaded) return;
  ...
  private resolveMaterialIconName(name: string, isDir: boolean): string | undefined {
    if (!this._materialMapping) return undefined;
  ...
  private async getMaterialIconUrl(name: string, isDir: boolean): Promise<string | undefined> {
    if (!this._materialExtPath || !this._materialMapping) return undefined;
  ...
  async refresh(): Promise<void> {
    ...
    if (!workspaceRoot) {
      (this._view as unknown as { title?: string }).title = "Project Size";
      this._view.webview.html = this.getPlaceholderHtml("Aucun workspace ouvert");
      return;
    }

    const rootName = path.basename(workspaceRoot);
    (this._view as unknown as { title?: string }).title = "Project Size · calcul...";
    ...
    void (async () => {
      try {
        ...
        (this._view as unknown as { title?: string }).title = `Project Size · ${totalText}`;
      } catch (error) {
  ...
  private async scanDirectory(dirPath: string): Promise<SizeEntry[]> {
  ...
  private static readonly TOP_FILES_SKIP_DIRS = new Set([
  ...
  private async scanAllFiles(dirPath: string, maxFiles = 20000, depth = 0): Promise<SizeEntry[]> {
    const results: SizeEntry[] = [];
    if (depth > 12 || results.length > maxFiles) return results;
    await this.loadMaterialIcons();
    ...
    while (stack.length && results.length < maxFiles) {
      ...
      for (const dirent of dirents) {
        ...
        if (dirent.isDirectory()) {
          if (WorkspaceSizesWebviewProvider.TOP_FILES_SKIP_DIRS.has(dirent.name)) continue;
  ...
  private async getTopFiles(rootPath: string, limit = 50): Promise<SizeEntry[]> {
    const all = await this.scanAllFiles(rootPath);
    return all.sort((a, b) => b.size - a.size).slice(0, limit);
  }

  private async buildTreeHierarchy(rootPath: string, maxDepth = 3): Promise<{
    name: string; path: string; size: number; isDir: boolean; children?: unknown[];
  }> {
    const buildNode = async (dirPath: string, name: string, depth: number): Promise<{
      name: string; path: string; size: number; isDir: boolean; children?: unknown[];
    }> => {
      const node: { name: string; path: string; size: number; isDir: boolean; children?: unknown[] } = {
        name,
        path: dirPath,
        size: 0,
        isDir: true,
      ...
      let dirents: Dirent[];
      try { dirents = await fs.readdir(dirPath, { withFileTypes: true }); } catch { return node; }

      const children: Array<{ name: string; path: string; size: number; isDir: boolean; children?: unknown[] }> = [];
      for (const dirent of dirents) {
        if (dirent.isSymbolicLink()) continue;
        const fullPath = path.join(dirPath, dirent.name);
        if (dirent.isDirectory()) {
          if (depth >= maxDepth) {
            try { const { total } = await getDirectorySizeBytes(fullPath); node.size += total; } catch { /* skip */ }
            continue;
          }
  ...
  private async detectCleanupTargets(rootPath: string): Promise<Array<SizeEntry & { suggestion: string }>> {
    const results: Array<SizeEntry & { suggestion: string }> = [];
    await this.loadMaterialIcons();
    ...
    while (queue.length && count < 500) {
      ...
      for (const dirent of dirents) {
        ...
        if (WorkspaceSizesWebviewProvider.CLEANUP_PATTERNS.includes(dirent.name)) {
          const fullPath = path.join(dir, dirent.name);
          let size = 0;
          try { const { total } = await getDirectorySizeBytes(fullPath); size = total; } catch { size = 0; }
  ...
  private async deleteCleanupTargets(paths: string[]): Promise<void> {
    let deleted = 0;
    let freed = 0;
    for (const p of paths) {
      try {
        ...
        vscode.window.showErrorMessage(`Impossible de supprimer ${path.basename(p)}: ${(error as Error).message}`);
      }
    }
  ...
  private async exportCsv(rootPath: string): Promise<void> {
    ...
    try {
      ...
      vscode.window.showErrorMessage(`Export échoué: ${(error as Error).message}`);
    }
  }

  private static readonly IMAGE_EXTS = [".png", ".jpg", ".jpeg", ".gif", ".webp", ".svg", ".ico", ".bmp", ".avif"];
  private static readonly VIDEO_EXTS = [".mp4", ".webm", ".mov", ".ogv", ".mkv"];
  private static readonly AUDIO_EXTS = [".mp3", ".wav", ".flac", ".ogg", ".m4a", ".aac", ".opus"];
  private static readonly CODE_EXTS = [".js", ".jsx", ".ts", ".tsx", ".mjs", ".cjs", ".py", ".rb", ".go", ".rs", ".java", ".kt", ".c", ".cc", ".cpp", ".h", ".hpp", ".cs", ".swift", ".php", ".sh", ".bash", ".zsh", ".fish", ".ps1", ".lua", ".r", ".scala", ".clj", ".ex", ".exs", ".erl", ".hs", ".elm", ".dart", ".vue", ".svelte", ".sql", ".graphql", ".gql", ".proto", ".twig", ".pug", ".hbs", ".ejs"];
  ...
  private async openSmartPreview(filePath: string): Promise<void> {
    ...
    try {
      ...
      if (stat.size > 50 * 1024 * 1024 && !WorkspaceSizesWebviewProvider.IMAGE_EXTS.includes(ext) && ext !== ".pdf") {
        const action = await vscode.window.showWarningMessage(
          `${name} fait ${formatBytes(stat.size)}. Aperçu potentiellement lent.`,
          "Aperçu quand même",
          "Annuler"
        );
        if (action !== "Aperçu quand même") return;
      }
    } catch {
      // ignore
    }
    ...
    try {
      ...
      panel.webview.html = `<!DOCTYPE html><html><body style="font-family:var(--vscode-font-family);padding:20px;color:var(--vscode-errorForeground)"><h2>Erreur d'aperçu</h2><pre>${escapeHtml((error as Error).message)}</pre></body></html>`;
    }
  ...
  private previewShell(title: string, bodyHtml: string, cspSource: string, extraHead = "", script = ""): string {
    const nonce = getNonce();
    return `<!DOCTYPE html>
<html lang="fr">
<head>
...
<body>
  ...
  private async buildImagePreview(filePath: string, panel: vscode.WebviewPanel): Promise<string> {
  ...
  private async buildHtmlPreview(filePath: string, panel: vscode.WebviewPanel): Promise<string> {
    const uri = panel.webview.asWebviewUri(vscode.Uri.file(filePath));
    const name = path.basename(filePath);
    const stat = await fs.stat(filePath);
    ...
    return this.previewShell(name, bodyHtml, panel.webview.cspSource, "",
    `document.getElementById('meta').textContent = '${escapeHtml(formatBytes(stat.size))}';`);
  }
  ...
  private async buildPhpPreview(filePath: string, panel: vscode.WebviewPanel): Promise<string> {
    ...
    `, panel.webview.cspSource, "",
    `document.getElementById('meta').textContent = '${escapeHtml(previewUrl)}';`);
  }

  private async buildMarkdownPreview(filePath: string, panel: vscode.WebviewPanel): Promise<string> {
    const content = await fs.readFile(filePath, "utf-8");
    const stat = await fs.stat(filePath);
    const name = path.basename(filePath);
  ...
  private async buildPdfPreview(filePath: string, panel: vscode.WebviewPanel): Promise<string> {
    const uri = panel.webview.asWebviewUri(vscode.Uri.file(filePath));
    const name = path.basename(filePath);
    const stat = await fs.stat(filePath);
    return this.previewShell(name, `
  ...
  private async buildJsonPreview(filePath: string, panel: vscode.WebviewPanel): Promise<string> {
  ...
  private async buildCsvPreview(filePath: string, panel: vscode.WebviewPanel, tsv: boolean): Promise<string> {
    const raw = await fs.readFile(filePath, "utf-8");
    const stat = await fs.stat(filePath);
    const name = path.basename(filePath);
    const sep = tsv ? "\t" : ",";
      ...
      <div style="padding:12px;">
        ...
        <div style="overflow:auto;border:1px solid rgba(255,255,255,0.1);border-radius:6px;">
          <table style="border-collapse:collapse;font-size:12px;width:100%;">
            <thead>
              <tr style="background:#0f0f12;position:sticky;top:0;">${rows[0]?.map((c:string) => `<th style="padding:6px 10px;text-align:left;border-bottom:1px solid rgba(255,255,255,0.1);color:#fff;">${escapeHtml(c)}</th>`).join("")}</tr>
            </thead>
            <tbody>
              ${rows.slice(1).map((row) => `<tr style="border-bottom:1px solid rgba(255,255,255,0.04);">${row.map((c:string) => `<td style="padding:5px 10px;">${escapeHtml(c)}</td>`).join("")}</tr>`).join("")}
            </tbody>
          </table>
        </div>
      </div>
  ...
  private async buildVideoPreview(filePath: string, panel: vscode.WebviewPanel): Promise<string> {
    const uri = panel.webview.asWebviewUri(vscode.Uri.file(filePath));
    const name = path.basename(filePath);
    const stat = await fs.stat(filePath);
    return this.previewShell(name, `
  ...
  private async buildAudioPreview(filePath: string, panel: vscode.WebviewPanel): Promise<string> {
    const uri = panel.webview.asWebviewUri(vscode.Uri.file(filePath));
    const name = path.basename(filePath);
    const stat = await fs.stat(filePath);
    return this.previewShell(name, `
  ...
  private async buildCodePreview(filePath: string, panel: vscode.WebviewPanel, notice = ""): Promise<string> {
    const raw = await fs.readFile(filePath, "utf-8");
    const stat = await fs.stat(filePath);
    const name = path.basename(filePath);
  ...
  private async buildTextPreview(filePath: string, panel: vscode.WebviewPanel): Promise<string> {
    const raw = await fs.readFile(filePath, "utf-8");
    const stat = await fs.stat(filePath);
    const name = path.basename(filePath);
  ...
  private async buildArchivePreview(filePath: string, panel: vscode.WebviewPanel): Promise<string> {
    const name = path.basename(filePath);
    const ext = path.extname(filePath).toLowerCase();
    const stat = await fs.stat(filePath);
    let listing = "";
    ...
    try {
      ...
      listing = `Impossible de lister l'archive: ${(e as Error).message}`;
    }
  ...
  private async buildStructuredPreview(filePath: string, panel: vscode.WebviewPanel): Promise<string> {
    const raw = await fs.readFile(filePath, "utf-8");
    const stat = await fs.stat(filePath);
    const name = path.basename(filePath);
    const ext = path.extname(filePath).toLowerCase();
  ...
  private getHtml(cspSource: string, rootPath: string, rootName: string, filter: string, totalBytes: number): string {
    const nonce = getNonce();
    return `<!DOCTYPE html>
<html>
<head>
...
async function createPreviewTooltip(uri: vscode.Uri, bytes: number): Promise<vscode.MarkdownString> {
...
export function activate(context: vscode.ExtensionContext) {
  ...
  try {
    ...
    sizeDecorLog(`[activate] registerKanban THREW: ${(e as Error).message}\n${(e as Error).stack}`);
  }

  // Gitignore decoration provider
  const workspaceRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
  let gitignoreProvider: GitignoreDecorationProvider | undefined;
  if (workspaceRoot) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(PROJECT_NOTES_VIEW_ID, notesProvider)
  );
  context.subscriptions.push(
    vscode.window.registerTreeDataProvider("agentHistoryView", agentHistoryProvider)
  );

  // Dedicated size tree: descriptions are readable and aligned after names.
  let sizeExplorerProvider: SizeExplorerProvider | undefined;
  let nativeSizeDecorationProvider: NativeSizeDecorationProvider | undefined;
  let workspaceSizesProvider: WorkspaceSizesWebviewProvider | undefined;
  try {
    ...
    context.subscriptions.push(
      vscode.window.registerWebviewViewProvider("workspaceSizesView", workspaceSizesProvider)
    );
    context.subscriptions.push(
    ...
    const readShowSizesConfig = () => {
      ...
      const enabled = cfg.get<boolean>("showSizes", true);
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand("pkvsconf.sizeExplorerRefresh", () => {
        sizeExplorerProvider?.refresh();
    ...
    let sizeDebounce: NodeJS.Timeout | undefined;
    const refreshSizesFor = (uri: vscode.Uri) => {
    ...
    sizeDecorLog(`[size init] THREW: ${(e as Error).message}\n${(e as Error).stack}`);
  }
  ...
  const checkStagedFilesForSecretsAutomatically = async () => {
    ...
    if (result.hasSecrets && result.files.length > 0) {
      ...
      const totalSecrets = result.files.reduce(
        (sum, file) => sum + file.secrets.length,
        0
      );
      ...
      const choice = await vscode.window.showWarningMessage(
        `🔐 ${totalSecrets} secret(s) détecté(s) dans les fichiers staged : ${fileList}${moreFiles}`,
        "Voir les secrets",
      ...
      if (choice === "Voir les secrets") {
        ...
        vscode.window.showInformationMessage(
          `${result.files.length} fichier(s) ajouté(s) au .gitignore et retiré(s) du staging.`
        );
      }
    }
  };
  ...
  const setupGitStagingWatcher = async () => {
    ...
    for (const repo of gitApi.repositories) {
      ...
      if (repo.state && typeof (repo.state as any).onDidChange === "function") {
        const disposable = (repo.state as any).onDidChange(() => {
    ...
    gitApi.onDidOpenRepository((repo: GitRepository) => {
      if (repo.state && typeof (repo.state as any).onDidChange === "function") {
        const disposable = (repo.state as any).onDidChange(() => {
          void checkStagedFilesForSecretsAutomatically();
        });
  ...
  const launchpadAddFolderCmd = vscode.commands.registerCommand(
  ...
  const launchpadRemoveCmd = vscode.commands.registerCommand(
    "pkvsconf.launchpadRemove",
    async () => {
      await removeProjectFromLaunchpad();
      await refreshLaunchpadViews();
    }
      ...
      if (!uri) {
        ...
        if (repos.length === 1) {
          ...
          const pick = await vscode.window.showQuickPick(
            repos.map((r) => ({ label: r.label, description: r.rootUri.fsPath, repo: r })),
            { placeHolder: "Reveal which repository?" }
          );
  ...
  const codexCaptureResumeCmd = vscode.commands.registerCommand(
    "pkvsconf.codexCaptureResume",
    async () => {
      const clipboard = await vscode.env.clipboard.readText();
      const command = extractCodexResumeCommand(clipboard);
      if (!command) {
        vscode.window.showWarningMessage(
          'Presse-papiers invalide. Copiez quelque chose comme "codex resume xxx" puis relancez la commande.'
        );
        return;
      }
  ...
  const codexSaveResumeCmd = vscode.commands.registerCommand(
    "pkvsconf.codexSaveResume",
    async () => {
  ...
  const codexPickResumeCmd = vscode.commands.registerCommand(
    "pkvsconf.codexPickResume",
    async () => {
  ...
  const agentHistoryAddFromClipboardCmd = vscode.commands.registerCommand(
    "pkvsconf.agentHistoryAddFromClipboard",
    async () => {
  ...
  const agentHistoryRunCmd = vscode.commands.registerCommand(
    "pkvsconf.agentHistoryRun",
    async (entry?: AgentHistoryEntry) => {
      ...
      const target = entry ?? (
        await vscode.window.showQuickPick(
          ...
          { placeHolder: "Lancer quelle session ?" }
        )
      )?.entry;
  ...
  const agentHistoryClearCmd = vscode.commands.registerCommand(
    "pkvsconf.agentHistoryClear",
    async () => {
      const choice = await vscode.window.showWarningMessage(
        "Vider tout l'historique Agent ?",
        "Vider",
        "Annuler"
      );
  ...
  const codexPickRecentSessionCmd = vscode.commands.registerCommand(
    "pkvsconf.codexPickRecentSession",
    async () => {
      const sessions = await readCodexSessionIndex();
      if (!sessions.length) {
        vscode.window.showInformationMessage(
          "Aucune session Codex trouvée dans ~/.codex/session_index.jsonl."
        );
        return;
      }
  ...
  const codexSuggestSessionCmd = vscode.commands.registerCommand(
    "pkvsconf.codexSuggestSession",
    async () => {
  ...
  const commitWithSecretCheckCmd = vscode.commands.registerCommand(
    "pkvsconf.commitWithSecretCheck",
    async () => {
      const result = await scanStagedFilesForSecrets();
...
<body>
  ...
  const createSkillsSymlinkCmd = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      type SymlinkState = "created" | "updated" | "exists" | "skipped";
      ...
      let agentSymlinkState: SymlinkState;
      let inspiSymlinkState: SymlinkState;
      ...
      try {
        ...
        vscode.window.showErrorMessage(
          `Erreur lors de la création du lien .inspi: ${error}`
        );
        return;
      }
      ...
      try {
        ...
        vscode.window.showWarningMessage(
          `Link des fichiers agent en racine impossible: ${error}`
        );
      }
      ...
      const stateLabel: Record<SymlinkState, string> = {
        created: "créé",
        updated: "mis à jour",
        exists: "déjà présent",
